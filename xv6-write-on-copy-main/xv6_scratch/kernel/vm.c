...
pde_t* cowuvm(pde_t *pgdir, uint sz){
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    *pte &= ~PTE_W;
    ...
    // e.g. page table entry xxxxxxxxxxx (*pte)
    //                     ~ 11111111101 (~PTE_W)
    //                       xxxxxxxxx0x (result)
    
    if (*pte == 0) {
      panic("[handle_pagefault]\tPTE is 0!");
    }
...
int handle_pagefault(pde_t *pgdir, struct proc *proc, uint va, uint sz) {
  if (va > USERTOP){
    proc->killed = 1;
  ...
  char *va0 = PGROUNDDOWN(va);
  old_page = uva2ka(pgdir, va0);
  ...
  if(kgetrefcnt(old_page) > 1){
   ...
   // *pte &= 0xFFF;
    // pte      = 0000000000ffff
    // new_page = yyyyyyyyyy0000
    *pte =  PTE_U | PTE_W | PTE_P | (uint)new_page;
    kdecrement(old_page);
    // pte      = yyyyyyyyyyffff
    
  } 
  
  // even if only one reference, mark as writeable and flush TLB
  *pte |= PTE_W;
